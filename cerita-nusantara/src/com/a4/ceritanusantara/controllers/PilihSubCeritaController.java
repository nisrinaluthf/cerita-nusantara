...
/**
 * Kelas <code>PilihSubCeritaController</code> adalah controller yang memproses input
 * untuk screen untuk memilih subcerita.
 */
			...
			for(int i=0; i<subCeritaButtonBounds.length; i++){
				if(screen.isSubCeritaButtonPressed(i)){
					...
					if(OverlapTester.pointInRectangle(subCeritaButtonBounds[i], pos.x, pos.y)){
						...
						if(screen.getSubCerita(i).getTipe()==SubCerita.ADEGAN){
							app.setScreen(new AdeganScreen(app, (Adegan)(screen.getSubCerita(i))));
						}
