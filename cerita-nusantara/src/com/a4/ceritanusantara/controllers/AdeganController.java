...
/**
 * Kelas <code>AdeganController</code> adalah controller yang memproses input
 * untuk screen subcerita bertipe adegan.
 */
				...
				//if(adegan.getPrev()!=null){
					if(screen.previousButtonIsPressed()){
						...
						if(OverlapTester.pointInRectangle(previousButtonBounds, pos.x, pos.y)){
								screen.stopMusic();
								app.getScreen().dispose();
								app.setScreen(new AdeganScreen(app, adegan));
							//}
							/*
							else if(type==SubCerita.TAP_GAME){
								screen.stopMusic();
