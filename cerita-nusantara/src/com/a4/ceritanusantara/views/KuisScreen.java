...
/**
 * Kelas <code>KuisScreen</code> adalah kelas untuk mengatur tampilan screen kuis termasuk suara dan musiknya.
 * Kelas ini menerima input dari pengguna dan meneruskannya ke kelas KuisController.
 */
public class KuisScreen extends AbstractScreen{
		...
		batcher.begin();
			if(kuis.isGameOver()){
				...
				font36.drawWrapped(batcher, question.getOptions(0), 100, 
						290, opsiBackgroundTexture.getWidth()-25);
				
				font36.drawWrapped(batcher, question.getOptions(1), 100, 
						130, opsiBackgroundTexture.getWidth()-25);
	...
	public void dispose(){
		...
		soalBackgroundTexture.dispose();
		opsiBackgroundTexture.dispose();
		opsiBackgroundPressedTexture.dispose();
		gameOverBg.dispose();
